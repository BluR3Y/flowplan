...
class SetOperation(TransformOperation, type_id="set"):
    ...
    def _load_plugins(self):
        ...
        try:
            eps = entry_points()
            group_name = "batch_etl.set_transforms"
            ...
            for ep in candidates:
                if ep.name not in self._SET_OPERATION_REGISTRY:
                    try:
                        ...
                        if ep.name not in self._SET_OPERATION_REGISTRY and callable(plugin_fn):
                            self._SET_OPERATION_REGISTRY[ep.name] = plugin_fn
                        
                        log.debug(f"Loaded set transform plugin: {ep.name}")
                    except Exception as e:
                        log.warning(f"Failed to load set transform plugin {ep.name}: {e}")
        except Exception as e:
            log.debug(f"Plugin loading skipped/failed: {e}")
    
    def get_operation(self, name):
        """Retrieve a transform operation by name"""
        if name not in self._SET_OPERATION_REGISTRY:
            self._load_plugins()
        if name not in self._SET_OPERATION_REGISTRY:
            raise KeyError(f"Unknown set transform: {name}")
        
        return self._SET_OPERATION_REGISTRY[name]